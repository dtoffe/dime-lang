...
This repository contains a Wirthian/Pascal-like language, compiler and toolchain project implemented in Free Pascal.

Primary goals:

- Preserve working behavior while evolving the compiler incrementally
- Favor small, clear, Wirthian data structures over heavy abstractions
- Avoid external dependencies unless explicitly approved
- Keep bootstrap feasibility in mind
- Prefer surgical parser/compiler changes over rewrites
- Verify compiler behavior with small source programs and expected p-code/MIR output where possible

When changing compiler internals:

- Do not silently change language semantics
- Do not rewrite parser/codegen architecture unless explicitly requested
- Preserve existing p-code behavior unless the task is specifically about replacing it
- Document any semantic assumption in the task report
...
- Never commit locally unless I explicitly request a commit or the execution environment requires a commit for review.

- Never amend or rewrite existing commits unless explicitly authorized.
